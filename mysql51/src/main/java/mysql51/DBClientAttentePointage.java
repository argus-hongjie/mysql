...
public class DBClientAttentePointage {
	...
	public void majArgusDemandePointage(String uniteSource, Integer idSource, String titre, 
		...
		try(Connection connection =  MysqlDataSourceManager.getInstance().getConnection()) {
			String sqlMajArgusDemandePointage = "INSERT INTO `argus-bulletinage`.`argus_demande_pointage` (UNIQUE_KEY,`DISPLAY`,`CREATE_USER`,`CREATE_DATE`,`MODIFY_USER`,`MODIFY_DATE`,`STATUT`,`UNITE_SOURCE`,`ID_SOURCE`,`TITRE`,`UTILISATEUR`,`CODE_BARRE`,`CHEMIN_FEUILLE_POINTAGE`,`DATE_PARUTION`,`DATE_FACIALE`,`NOMBRE_PAGE`,`MESSAGE_POINTAGE`,`DATE_POINTAGE`,`PREDICTED_ISSUE`,`STATUT_LABEL`,`DATE_PARUTION_LABEL`,`DATE_POINTAGE_LABEL`,`NUMERO_PUBLICATION`,`NUMERO_LOT`,`MODE_APPROVISIONNEMENT`,`MODE_APPROVISIONNEMENT_LABEL`,`TYPE_DE_TRAITEMENT`,`INFO_SUPP`,`IS_REJEU`,`ID_INSTANCE_APPRO`,`TRAITEMENT_ID`,`TRAITEMENT_LABEL`) SELECT COALESCE(max(unique_key), 0)+1 ,NULL, ''ESB'', DATE_FORMAT(NOW(), ''%Y%m%d%H%i%s''), NULL, NULL, ''1'', :unite_source, :id_source, :titre, NULL, :code_barre, :chemin_feuille_pointage, :date_parution, :date_faciale, NULL, NULL, NULL, NULL, ''A compléter'', '''', NULL, '''', ''0'', ''6'', ''Flux numérique'', ''1'', NULL, NULL, ''0'', ''2'', ''TraitementPDFArticle'' FROM `argus-bulletinage`.argus_demande_pointage";
	...
	public Map<String, ?> callWsEsb(Integer identifiantUniteSource, String dateParution, String dateFaciale, String codeBarres, String cheminFeuillePointage, Boolean dateFiable) throws SQLException {
		try(Connection connection =  MysqlDataSourceManager.getInstance().getConnection()) {
            String procedureCall = "CALL `argus-bulletinage`.`ps_maj_reception`(:identifiant_unite_source, :date_parution, :date_faciale, :code_barres, :chemin_feuille_pointage, 0, '''', 0, :date_fiable, 0, ''ESB'', :@code_retour, :@message_retour)";
